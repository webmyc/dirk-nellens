...
export default function Navigation() {
  ...
  return (
    <>
          ...
          <Link href="/" className="flex items-center gap-4 group">
            <div
              className={`relative flex-shrink-0 transition-all duration-300 ease-in-out ${isScrolled ? 'w-[58px] h-[58px]' : 'w-[62px] h-[62px] md:w-[82px] md:h-[82px]'
                >
                <Image
                  src={isHome && !isScrolled ? "/logo/logo-symbol-white.png" : "/logo/logo-symbol-black.png"}
      ...
      <button
        className={`lg:hidden p-2 transition-colors ${isHome && !isScrolled ? 'text-white hover:text-white/80' : 'text-[#2C2C2C] hover:text-[#D96C40]'}`}
